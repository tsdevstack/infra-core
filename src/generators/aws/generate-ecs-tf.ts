/**
 * Generate ecs.tf with ECS cluster, task definitions, and services
 *
 * Creates:
...
export function generateEcsTf(): string {
  return `# ECS Cluster, Services, and Workers
...
resource "aws_ecs_task_definition" "worker" {
  ...
  container_definitions = jsonencode([{
    ...
    # Override Dockerfile CMD to run worker entrypoint instead of main API server
...
# Workers poll Redis — minInstances >= 1 enforced by CLI
# =============================================================================
...
resource "aws_ecs_service" "worker" {
  ...
  desired_count   = each.value.minInstances
  launch_type     = "FARGATE"
...
# =============================================================================
# Worker Auto Scaling
# Workers scale based on CPU utilization within their configured min/max range
# =============================================================================
...
# =============================================================================
# Task Definitions (Next.js)
# Same Docker image as built by CLI, uses Dockerfile CMD (no command override)
# =============================================================================
...
resource "aws_ecs_task_definition" "nextjs" {
  ...
  memory                   = each.value.memory
  execution_role_arn       = aws_iam_role.ecs_execution.arn
  task_role_arn            = aws_iam_role.nextjs_task[each.key].arn
  ...
  container_definitions = jsonencode([{
    ...
    healthCheck = {
      command     = ["CMD-SHELL", "node -e \\"fetch('http://localhost:8080/health').then(r=>{if(!r.ok)throw r.status}).catch(()=>process.exit(1))\\""]
      interval    = 30
      timeout     = 5
      retries     = 3
      startPeriod = 60
...
# =============================================================================
# ECS Services (Next.js)
# Routed via ALB host-based rules on port 443 (not through Kong)
# =============================================================================
...
resource "aws_ecs_service" "nextjs" {
  ...
  name            = each.key
  cluster         = aws_ecs_cluster.main.id
  task_definition = aws_ecs_task_definition.nextjs[each.key].arn
...
resource "aws_appautoscaling_target" "nextjs" {
  ...
  max_capacity       = each.value.maxInstances
  min_capacity       = max(each.value.minInstances, 1)
  resource_id        = "service/\${aws_ecs_cluster.main.name}/\${aws_ecs_service.nextjs[each.key].name}"
  scalable_dimension = "ecs:service:DesiredCount"
  service_namespace  = "ecs"
...
resource "aws_appautoscaling_policy" "nextjs_cpu" {
  ...
  name               = "\${var.project_name}-\${each.key}-cpu"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.nextjs[each.key].resource_id
  scalable_dimension = aws_appautoscaling_target.nextjs[each.key].scalable_dimension
  service_namespace  = aws_appautoscaling_target.nextjs[each.key].service_namespace
  ...
  depends_on = [aws_appautoscaling_target.nextjs]
}
`;
}
